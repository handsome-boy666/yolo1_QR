...
基于 YOLOv1 改进的二维码检测，支持训练/评估/预测/实时摄像头。单类目标，网络简化，易上手。

如果需要用于其他单类目标检测任务，只需替换数据集即可。本项目效果展示如下：
<p align="center">
...
由于本项目仅检测二维码（C=0，不计算分类损失），损失函数公式简化如下：
...
*   **预训练权重**: 提供已训练好的模型权重：[百度网盘链接](https://pan.baidu.com/s/1GqtT--_dBsO1hufyjjAWZQ?pwd=3e2e)  提取码：`3e2e`
...
* **评价**：精确率极高（误检极少）、召回率尚可（存在约 12 % 漏检）、定位精度良好、阈值鲁棒性稳定。

* **实时性能**
  - GPU：≥ 30 FPS，可满足实时需求。  
  - CPU：帧率明显下降，后续可进一步优化推理速度。

* **缘部署提示**：权重文件较大，树莓派等内存受限设备需提前评估内存占用。

另外：对于单类别检测任务，可直接替换数据集，即可使用本项目进行训练和预测。
